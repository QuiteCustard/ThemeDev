...
function samsTheme_customize_register($wp_customize)
{
    ...
    // Colour section in theme option panel
    // Layout colours
    $wp_customize->add_section('samsTheme_colour_options', array(
    ...
    $wp_customize->add_setting('samsTheme_colour_body', array(
        'default' => 'ffffff',
        'type' => 'option',
    ...
    $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, 'samsTheme_colour_body', array(
        'label' => esc_html__('Body Colour', 'samsTheme') ,
        'section' => 'samsTheme_colour_options',
        'settings' => 'samsTheme_colour_body',
    )));
...
?>
    ...
    // Add control for radio buttons
    $wp_customize->add_control('samsTheme_radio_control', array(
        'label' => esc_html__('Would you like a border?', 'samsTheme') ,
        'section' => 'samsTheme_colour_options',
    ...
    $wp_customize->add_setting('samsTheme_default_option', array(
        'default' => '123456',
        'sanitize_callback' => 'sanitize_hex_color_no_hash',
        'sanitize_js_callback' => 'maybe_hash_hex_color',
    ));

    // Add  control that holds the default value
    $wp_customize->add_control('samsTheme_default_option', array(
...
function samsTheme_customization_css_colours()
{
    ...
    if (get_option('samsTheme_radio_control') == 'default') {
        $border_colour = get_option('samsTheme_default_option');
    }
    else {
        $border_colour = get_option('samsTheme_custom_option');
    }
